...
import { getLibro, getCierre } from "@/lib/grades/queries";
...
export default async function CalificacionesPage({
  ...
  if (!sede || !anio) {
    return (
      <div>
        <PageHeader title="Calificaciones" description="Libro de calificaciones." />
        <EstructuraVacia />
      </div>
    );
  }

  const [niveles, grados, secciones, asignaturas, pensum, periodos, esquemas] =
    await Promise.all([
      getNiveles(sede.id),
      getGrados(sede.id),
  ...
  const esquema = esquemas.find((e) => e.activo) ?? esquemas[0] ?? null;
  const componentes = esquema ? await getComponentes(esquema.id) : [];

  // Secciones numéricas (Primaria/Secundaria).
  ...
  // Asignaturas del pénsum del grado de la sección.
  ...
  const [roster, libro, cierre] = await Promise.all([
    seccionSel
      ? getEstudiantesDeSeccion(seccionSel, anio.id)
      : Promise.resolve([]),
    seccionSel && asignaturaSel && periodoSel
      ? getLibro(seccionSel, asignaturaSel, periodoSel)
      : Promise.resolve([]),
    seccionSel && asignaturaSel && periodoSel
  ...
  return (
    <div>
      <PageHeader
        ...
        description={`${sede.nombre} · Libro del docente`}
      />
      <LibroView
        canCerrar={canCerrar}
